...
for lm_beta, ins_gamma, T in params:
    ...
    for i, (poses, labels) in enumerate(testdataloader):
        ...
        votes = []
        for i in range(T):
            single_pass = F.log_softmax(all_logits[i], dim=-1).permute(1,0,2)  # [B, T, C] → [T, B, C] if needed
            pred = decoder_dec.beam_decode_trans(single_pass[:,0,:].cpu(), beam_size, model, poses, beta=lm_beta, gamma=ins_gamma)
            votes.append(''.join(pred))
        current_preds = Counter(votes).most_common(1)[0][0]  # Get the most common prediction
    ...
    with open('vote_results2.csv', mode='a', newline='') as file:
